...
export default function AdminPortal({ user, onLogout }) {
  ...
  // Refresh clashes from backend
  const refreshClashes = async () => {
    try {
      const data = await getClashes();
  ...
  const handleResolveAll = async () => {
    try {
      ...
      await refreshClashes();
      showToast("✅ All clashes resolved!");
    } catch (e) { showToast("❌ " + e.message, "error"); }
  };
    ...
    <div style={{ display:"flex", height:"100vh", background:"#0a0a0a", overflow:"hidden" }}>
      ...
      {toast && (
        <div style={{
          ...
          boxShadow:"0 8px 30px rgba(0,0,0,.5)", maxWidth:360,
      ...
      <aside style={{ width:218, minWidth:218, background:"#0d0d0d", borderRight:"1px solid rgba(255,255,255,.05)", display:"flex", flexDirection:"column" }}>
        ...
        {/* Publish & Download buttons in sidebar */}
        {entries.length > 0 && (
          <div style={{ padding:"10px 10px", borderTop:"1px solid rgba(255,255,255,.05)", display:"flex", flexDirection:"column", gap:6 }}>
            <button
          ...
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            ...
            <div>
              <h2 style={{ fontSize:15, fontWeight:700, color:"#fff" }}>
                {page === "upload" ? "Upload Timetable" : page === "clashes" ? "⚠️ AI Clash Detection" : "📅 Timetable View"}
              </h2>
          ...
          {page === "upload" && (
            <div className="fadein">
              ...
              >
                ...
                {analyzing ? (
                  <>
                    ...
                    <h3 style={{ fontSize:20, fontWeight:700, color:"#fff", marginBottom:8 }}>Analyzing Timetable…</h3>
                    <p style={{ fontSize:13, color:"#6b7280", marginBottom:22 }}>Backend AI is parsing entries and detecting clashes</p>
                  ...
                  <>
                    ...
                    <p style={{ fontSize:13, color:"#6b7280", marginBottom:6 }}>Drag & drop your .xlsx file or click to browse</p>
                    <p style={{ fontSize:11, color:"#4b5563", marginBottom:28 }}>Backend will parse & detect all clashes automatically</p>
              ...
              <div className="card" style={{ padding:20, marginTop:22 }}>
                <div style={{ fontSize:13, fontWeight:600, color:"#fff", marginBottom:14 }}>📋 Entry Format & Workflow</div>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
                  {[
                    ["1. Upload","Drop .xlsx file","Backend parses all entries"],
                    ["2. Detect","AI finds clashes","Venue · Teacher · Batch"],
                    ["3. Resolve","Fix all conflicts","Individual or Auto-resolve all"],
                    ["4. Publish","Students can view","They can also download it"],
                  ].map(([step, title, desc]) => (
                ...
                <>
                  ...
                  <div className="card" style={{ overflow:"hidden" }}>
                    <div style={{ padding:"16px 20px", borderBottom:"1px solid rgba(255,255,255,.05)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                      ...
                      {pending > 0 && <button className="btn btn-green" onClick={handleResolveAll}>✨ Auto-Resolve All ({pending})</button>}
                    </div>
                    ...
                    {clashes.length === 0 ? (
                      <div style={{ padding:"70px 20px", textAlign:"center" }}>
                        ...
                        <p style={{ color:"#6b7280", fontSize:13 }}>Your timetable is clean — no conflicts found.</p>
                      </div>
                    ) : clashes.map((c, i) => (
                      <div key={i} className="clash-row" style={{ padding:"16px 20px", display:"flex", gap:14, alignItems:"flex-start" }}>
                        <div style={{ width:42, height:42, borderRadius:13, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0,
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ display:"flex", gap:7, alignItems:"center", flexWrap:"wrap", marginBottom:7 }}>
                            <span className={`tag tag-${c.status==="resolved" ? "ok" : c.type}`}>{c.status==="resolved" ? "✓ Resolved" : c.label}</span>
                          ...
                          <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom: c.status==="pending" ? 10 : 0 }}>
                            {c.entries.map((e, ei) => (
                              <div key={ei} style={{ background:"#1a1a1a", border:"1px solid rgba(255,255,255,.06)", borderRadius:9, padding:"5px 10px", fontSize:11, display:"flex", gap:5, alignItems:"center" }}>
                          ...
                          {c.status === "pending" && (
                            <div style={{ background:"rgba(124,58,237,.08)", border:"1px solid rgba(124,58,237,.2)", borderRadius:10, padding:"9px 13px", fontSize:12, color:"#c4b5fd", lineHeight:1.5 }}>
                              💡 <strong>AI Suggestion:</strong> {c.suggestion}
                            </div>
                          )}
                        </div>
                        <div style={{ flexShrink:0 }}>
                          {c.status === "pending"
                            ? <button className="btn btn-green" onClick={() => handleResolve(i)}>✓ Resolve</button>
